...
def get_task_queue() -> TaskQueue:
    ...
    if env == "production":
        ...
        if not gcp_project_id:
            raise ValueError("GCP_PROJECT_ID environment variable is required for production")
            
        service_url = os.environ.get("SERVICE_URL")
        if not service_url:
            raise ValueError("SERVICE_URL environment variable is required for production")
        ...
        try:
            ...
            logger.warning("Falling back to LocalTaskQueue despite production environment")
