    ...
    try:
        ...
        if payload.get("generate_image", True):
            ...
            if image_result and "url" in image_result:
                ...
                logger.info(f"Added image to story {story_id}: {image_result.get('url')}")
...
async def generate_story_snippet(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(f"Generating snippet for story {story_id} with strategy {strategy_name}")
    ...
    try:
        ...
        if payload.get("generate_image", True):
            ...
            if image_result and "url" in image_result:
                content["image"] = image_result
                logger.info(f"Added image to snippet in story {story_id}: {image_result.get('url')}")
