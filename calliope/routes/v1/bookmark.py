...
async def create_frame_bookmark(
    ...
    story = (
        await Story.objects()
        .where(Story.cuid == bookmark_request.story_id)
        .first()
        .run()
    )
    ...
    existing_bookmark = (
        await StoryFrameBookmark.objects()
        .where(
...
async def list_frame_bookmarks(
    ...
    ).where(StoryFrameBookmark.sparrow.id == sparrow_state.id)

    # Add story filter if provided
    if story_id:
        story = await Story.objects().where(Story.cuid == story_id).first().run()

        if not story:
            raise HTTPException(status_code=404, detail="Story not found")

        query = query.where(StoryFrameBookmark.frame.story.id == story.id)
    ...
    for bookmark in bookmarks:
        # Get the frame info
        frame = (
            await StoryFrame.objects(StoryFrame.image)
            .where(StoryFrame.id == bookmark.frame.id)
            .first()
            .run()
        )
...
@router.get(
    "/frame/story/{story_id}/frame/{frame_number}",
    response_model=StoryFrameBookmarkResponse,
)
async def get_frame_bookmark(
    ...
    frame = (
        await StoryFrame.objects()
        .where(StoryFrame.story.id == story.id, StoryFrame.number == frame_number)
        .first()
        .run()
    )
